  ...
  def list_of_train_cars
    @trains.each do |train|
      train.show_wagons do |wagon|
        puts "Train № #{train.number} : Wagon №- #{wagon.number};Wagon type- #{wagon.type}; Free seats- #{wagon.free_seats}; Busy seats- #{wagon.busy_seats}  " if wagon.type == 'passenger'
        puts "Train № #{train.number}: Wagon №- #{wagon.number};Wagon type- #{wagon.type}; Free volume- #{wagon.free_volume}; Busy volume- #{wagon.busy_volume} " if wagon.type == 'cargo'
      end
    end
  end
end
